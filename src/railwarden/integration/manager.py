...
from railwarden.config.models import ProjectConfig, ValidationCommand
...
from railwarden.validation.runner import cleanup_new_untracked, run_validation_suite


def _acquire_lock(lock_file: TextIO) -> None:
    if os.name == "nt":
    ...
    *,
    ...
    with lock_path.open("w", encoding="utf-8") as lock_file:
        ...
        if not branch_exists(repository, config.integration_branch):
            raise GitError(
                f"Integration branch does not exist: {config.integration_branch}"
            )
        switched = False
