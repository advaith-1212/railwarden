...
from railwarden.compat import (
    ...
    PREFERRED_WORKTREE_DIR,
    compatible_project_path,
...
from railwarden.git import discover_repo
from railwarden.validation.commands import parse_validation_argv
...
def load_project_config(repository_root: Path) -> ProjectConfig:
    config_dir = project_config_directory(repository_root)
    ...
    board = str(project.get("board", f"railwarden-{name}"))
    ...
    return ProjectConfig(
        ...
        integration_branch=str(
            project.get("integration_branch", "integration/railwarden")
        ),
        worktree_root=compatible_project_path(
            repository_root,
            str(project.get("worktree_root", "auto")),
            preferred_name=PREFERRED_WORKTREE_DIR,
            legacy_name=LEGACY_WORKTREE_DIR,
        ),
        runtime_directory=compatible_project_path(
            repository_root,
            str(runtime.get("directory", ".railwarden-runtime")),
            preferred_name=PREFERRED_RUNTIME_DIR,
            legacy_name=LEGACY_RUNTIME_DIR,
        ),
        board=board,
...
def load_work_packages(repository_root: Path) -> dict[str, WorkPackage]:
    path = project_config_directory(repository_root) / "work_packages.yaml"
    if not path.exists():
        return {}
...
def load_validation(repository_root: Path) -> tuple[ValidationCommand, ...]:
    path = project_config_directory(repository_root) / "validation.yaml"
    if not path.exists():
        return ()
    payload = _load_yaml(path)
