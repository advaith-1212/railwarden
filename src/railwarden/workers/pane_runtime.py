...
from railwarden.runtime.session import AgentInstance
...
def idle_pane_command(
    ...
    """Leave a plain shell ready for RailWarden to inject batch provider commands.
    ...
    label = (
        ...
        "RailWarden runs tasks here as visible CLI commands"
    )
...
def task_pane_command(
    *,
    ...
    return (
        f"cd {quoted_cwd} && "
        f"echo '[RailWarden] starting task in {quoted_cwd}' && "
        f"{quoted_argv}{stdin_redirect} 2>&1 | tee -a {quoted_log}; "
        f"echo '[RailWarden] task finished (exit='$?')'"
    )
