...
from railwarden.tmux.session import pane_alive, tmux
...
def pane_for_worker(
    ...
    candidates: list[str] = []
    if agent_id and agent_id not in _CONTROL_PANE_KEYS and agent_id in panes:
    ...
    if provider:
        for key, pane in panes.items():
            ...
            if (
                key == provider
                or key.startswith(f"{provider}-")
                or key.startswith(provider)
            ):
...
def launch_tmux_managed(
    ...
    *,
    ...
    try:
        atomic_write_text(
            script_path, _visible_runner_script(managed_command, pid_path, provider)
        )
...
banner = f"[RailWarden] starting {{payload['provider']}}: {{payload['argv']}}\\n"
...
with log_path.open("a", encoding="utf-8") as log:
    ...
    for line in process.stdout:
        ...
        log.flush()
    rc = process.wait()
    footer = f"[RailWarden] {{payload['provider']}} exited with {{rc}}\\n"
