...
def skill_directories(config: ProjectConfig) -> tuple[Path, Path]:
    return (
        project_config_directory(config.repository_root) / "skills",
...
def promote_runtime_skill(config: ProjectConfig, name: str) -> Path:
    ...
    if not runtime_path.exists():
        raise RailWardenError(f"Runtime skill does not exist: {name}")
    project_path = (
        project_config_directory(config.repository_root) / "skills" / f"{name}.md"
    )
