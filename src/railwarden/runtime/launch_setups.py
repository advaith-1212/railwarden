...
from railwarden.compat import environment_value, preferred_or_legacy_path
from railwarden.errors import ConfigurationError, RailWardenError
...
def setup_storage_root() -> Path:
    override = environment_value("RAILWARDEN_HOME")
    if override:
        return Path(override).expanduser().resolve()
    return preferred_or_legacy_path(
        Path.home() / ".railwarden", Path.home() / ".lfg"
    ).resolve()
    ...
    if "gpt" in setup.name.lower() and "gpt" in setup.model.lower():
        ...
        if name_nums and model_nums and name_nums[0] != model_nums[0]:
            raise ConfigurationError(
                f"Setup name '{setup.name}' implies a different model version than configured model '{setup.model}'."
            )
...
def write_runtime_env(path: Path, setup_names: list[str]) -> dict[str, str]:
    ...
    lines = ["# RailWarden runtime-only secrets. This file must remain ignored by git."]
...
def default_auth_env_var(provider: str, setup_name: str) -> str:
    ...
    return f"RAILWARDEN_{prefix}_{suffix}_API_KEY"
