...
from railwarden.validation.worker_result import load_worker_result
...
def normalize_result(
    ...
    *,
    ...
    source_path = Path(
        str(task.get("result_path", worker_result_path(workspace, task_id)))
    )
    runtime_path = Path(
        str(task.get("runtime_result_path", runtime_result_path(config, task_id)))
    )
    if source_path.exists():
        ...
        if not tracked_is_clean(workspace):
            raise RailWardenError(
                f"Cannot synthesize result for dirty worktree: {workspace}"
            )
        ...
        payload = {
            ...
            "status": "completed",
            "summary": "RailWarden synthesized result from clean committed work.",
    ...
    payload["normalized_by"] = "railwarden"
