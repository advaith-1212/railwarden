...
from railwarden.planning.pipeline import create_pending_plan, new_run_id
...
def execute_planning_job(config: ProjectConfig, run_id: str) -> None:
    ...
    path = planning_job_path(runtime_dir, run_id)
    if not path.exists():
        raise RailWardenError(f"Planning job not found: {run_id}")
    payload = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise RailWardenError(f"Invalid planning job: {path}")
    goal = str(payload.get("goal", "")).strip()
    if not goal:
        raise RailWardenError(f"Planning job {run_id} is missing a goal")
    try:
...
def _planning_worker_running(run_id: str) -> bool:
    try:
        completed = subprocess.run(
            ["pgrep", "-f", f"railwarden.cli.main planning-worker --run-id {run_id}"],
            text=True,
            capture_output=True,
            check=False,
        )
