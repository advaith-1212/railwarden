...
from railwarden.runtime.tasks import ensure_task
...
from railwarden.validation.commands import sanitize_planner_validation_commands
from railwarden.validation.contract import infer_scaffold_owned_paths
...
def build_planning_prompt(goal: str) -> str:
    return f"""You are the RailWarden planning architect.
...
def _extract_json(text: str) -> dict[str, Any]:
    ...
    if first == -1 or last == -1 or last < first:
        raise RailWardenError("Planner output did not contain a JSON object")
    payload = json.loads(stripped[first : last + 1])
    if not isinstance(payload, dict):
        raise RailWardenError("Planner output JSON must be an object")
    return payload


def build_structuring_prompt(goal: str, plan_markdown: str) -> str:
    return f"""You are converting an RailWarden planning response into machine-readable data.
...
def _normalize_work_packages(packages: Any) -> list[dict[str, Any]]:
    if not isinstance(packages, list):
        raise RailWardenError("Planner output requires work_packages list")
    normalized: list[dict[str, Any]] = []
    for raw in packages:
        if not isinstance(raw, dict):
            raise RailWardenError("Each work package must be an object")
        package_id = str(raw.get("id", "")).strip()
        if not package_id:
            raise RailWardenError("Each work package requires id")
...
def parse_planner_output(
    ...
    *,
    ...
    if not isinstance(plan_markdown, str) or not plan_markdown.strip():
        raise RailWardenError("Planner output requires plan_markdown")
...
def _planner_text(
    ...
    *,
    ...
    timeout = float(environment_value("RAILWARDEN_PLANNER_TIMEOUT_SECONDS") or "300")
    try:
    ...
    if completed.returncode != 0:
        raise RailWardenError(f"Planner failed: {completed.stderr.strip()}")
...
def approve_latest_plan(config: ProjectConfig) -> dict[str, Any]:
    ...
    config_dir = project_config_directory(config.repository_root)
    atomic_write_text(
        config_dir / "plan.md",
        (
...
def _resolve_owned_path_overlaps(
...
def _repo_path_candidates(repository_root: Path) -> list[str]:
    ...
    for path in repository_root.rglob("*"):
        if (
            ".git" in path.parts
            or ".railwarden-runtime" in path.parts
            or ".railwarden-worktrees" in path.parts
...
def _normalize_repo_path(value: str, candidates: list[str]) -> str:
    ...
    if not normalized:
        return normalized
    if normalized.startswith((".railwarden-worktrees/", ".lfg-worktrees/")):
