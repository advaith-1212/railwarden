...
def color_enabled() -> bool:
    if os.environ.get("NO_COLOR"):
        return False
    if environment_value("RAILWARDEN_PLAIN_UI"):
        return False
    return console.is_terminal


def use_interactive_select() -> bool:
    if environment_value("RAILWARDEN_PLAIN_PROMPTS"):
        return False
...
def prompt_text(default: str, label: str, *, hint: str = "") -> str:
    ...
    if use_interactive_select():
        ...
        if value is None:
            raise RailWardenError("Setup cancelled")
...
def prompt_choice(default: str, label: str, choices: dict[str, str]) -> str:
    ...
    if use_interactive_select():
        ...
        value = questionary.select(
            ...
            instruction="(Up/Down move, Enter to select)",
            style=style,
            use_indicator=True,
    ...
    if value in choices:
        return value
    raise RailWardenError(f"Expected one of: {', '.join(choices)}")
...
def prompt_bool(default: bool, label: str) -> bool:
    ...
    if value in {"0", "false", "no", "n"}:
        return False
    raise RailWardenError(f"Expected yes/no for {label}")
...
def launch_summary(
    *, session: str, profile_path: str, hermes_home: str, attach_hint: str
) -> None:
    ...
    console.print()
    console.print(
        Panel(table, title="[bold green]Factory launched[/]", border_style="green")
    )
    console.print()
...
def setup_summary_block(*, repository: str, configured: bool, created: bool) -> None:
    banner("RailWarden setup", subtitle=f"Repository: {repository}")
    if created:
        success("Created .railwarden project configuration")
    elif configured:
        info("Project configuration already exists")
    console.print()
    section("Next steps", hint="Run these before your first factory launch")
    console.print(
        "  [cyan]1.[/] [bold]warden doctor[/]     [dim]verify tools, credentials, MCP[/]"
    )
    console.print(
        "  [cyan]2.[/] [bold]warden context status[/]  [dim]populate context if needed[/]"
    )
    console.print(
        "  [cyan]3.[/] [bold]warden launch[/]     [dim]start the factory (guided wizard)[/]"
    )
