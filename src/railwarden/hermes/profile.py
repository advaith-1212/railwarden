...
from railwarden.compat import project_config_directory
from railwarden.config.models import ProjectConfig
from railwarden.errors import RailWardenError
from railwarden.hermes.azure import resolve_azure_hermes_config
from railwarden.runtime.launch_setups import load_setup_env, write_runtime_env
from railwarden.runtime.secrets import ensure_runtime_secrets_file
from railwarden.runtime.session import SessionProfile
...
def generate_hermes_profile(
    ...
    instructions_path = home / "railwarden-factory-instructions.md"
    soul_path = home / "SOUL.md"
    skill_path = home / "skills" / "railwarden-factory" / "SKILL.md"
    ...
    skill_dirs = [
        str(project_config_directory(config.repository_root) / "skills"),
    ...
    command = _railwarden_mcp_command()
...
def validate_hermes_orchestrator_config(
    ...
    if not isinstance(payload, dict):
        raise RailWardenError(f"Invalid Hermes config: {config_path}")
    model = payload.get("model")
    if not isinstance(model, dict):
        raise RailWardenError(f"Hermes config is missing model settings: {config_path}")
    ...
    if actual != expected:
        raise RailWardenError(
            ...
            f"{expected!r}. Re-run `warden launch` to regenerate Hermes config."
        )
    if expected == "azure-foundry":
        ...
        if not model.get("base_url"):
            raise RailWardenError(
                ...
                "setup saved AZURE_OPENAI_ENDPOINT and re-run `warden launch`."
            )
        if not model.get("api_mode"):
            raise RailWardenError(
                "Hermes Azure config is missing api_mode. Re-run `warden launch`."
            )
...
def _write_hermes_dotenv(home: Path, env: dict[str, str]) -> None:
    ...
    lines = [
        "# Hermes runtime secrets for RailWarden. This file must remain ignored by git."
    ]
...
def _railwarden_mcp_command() -> list[str]:
    return ["warden", "mcp", "serve"]
...
def _instructions(config: ProjectConfig, session: SessionProfile) -> str:
    ...
    return f"""# RailWarden Factory Runtime

You are Hermes, the accountable orchestrator for RailWarden's evented runtime.
RailWarden is authoritative for durable goals, tasks, events, DAG scheduling,
...
Event loop:
1. Observe `railwarden.state.snapshot` and `railwarden.events.tail`.
2. Diagnose the current runtime fact, especially `decision_required` events.
3. Choose one allowed RailWarden action.
4. Call the RailWarden MCP/CLI tool for that action.
5. Record the decision with `railwarden.decision.record`.
...
- Never mutate repository truth directly; act through RailWarden tools.
- Do not wait passively when RailWarden emits a recoverable failure.
...
- If implementation seems necessary, route it as an RailWarden work package.
...
- Wrapper quoting failures: avoid the same provider, hand off, and open adapter
  repair if needed.
- Missing result with a clean commit: call `railwarden.result.normalize`.
- Contract ownership or validation command gaps: repair the contract, asking
  only when scope materially expands.
- Branch divergence: reconcile only when RailWarden contract commits are the sole
...
Worker steering:
- Route implementation guidance to workers with `railwarden.worker.message`.
...
You are Hermes, the accountable orchestrator for RailWarden's agent factory.
...
Boundaries:
- RailWarden owns durable state, scheduling, validation, and integration.
...
- Steer workers only through `railwarden.worker.message`.
...
def _skill(instructions: str) -> str:
    return f"""---
name: railwarden-factory
description: Use RailWarden MCP tools to run this repository as a durable agentic development factory.
---
