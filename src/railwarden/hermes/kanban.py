...
from railwarden.config.models import (
    ProjectConfig,
    ProjectFiles,
    ValidationCommand,
    WorkPackage,
...
from railwarden.hermes.profile import hermes_executable
from railwarden.scheduler.classifier import package_branch
    ...
    def run(
        ...
        *,
        ...
        if self.executable is None:
            raise RailWardenError("Hermes executable is not installed.")
        ...
        if check and completed.returncode != 0:
            ...
            raise RailWardenError(
                f"Hermes command failed: {' '.join(command)}\n{detail}"
            )
        return result
    ...
    def json(
        ...
        *,
        ...
        if not isinstance(payload, dict):
            raise RailWardenError("Hermes JSON output must be an object.")
        return payload
...
def apply_bootstrap(config: ProjectConfig, adapter: HermesAdapter) -> BootstrapPlan:
    ...
    adapter.run(
        [
            ...
            config.name,
            "--description",
            f"RailWarden companion board for {config.repository_root}",
            "--switch",
        ],
        check=False,
    )
    ...
    for profile in plan.required_profiles:
        ...
        adapter.run(
            [
                ...
                "--clone",
                "--description",
                f"RailWarden Kanban worker profile for {config.name}.",
            ],
            check=False,
        )
    return plan
...
def apply_import_plan(
    ...
    for task in plan.tasks:
        ...
        if task_id is None:
            raise RailWardenError(
                f"Hermes create did not return a task id for {task.title}."
            )
...
def _task_plan(config: ProjectConfig, package: WorkPackage) -> KanbanTaskPlan:
    return KanbanTaskPlan(
        ...
        idempotency_key=f"railwarden:{config.name}:{package.package_id}",
...
def _task_body(config: ProjectConfig, package: WorkPackage) -> str:
    ...
    lines = [
        ...
        *_bullets(package.context_refs),
        ...
        "",
        "Hermes/RailWarden boundary:",
        "- RailWarden owns task lifecycle, run history, retries, handoffs, and validation.",
        "- Hermes Kanban is an optional projection and must route writes through RailWarden.",
    ]
...
def _safe_text(
    ...
    *,
    ...
    except (RailWardenError, OSError, subprocess.TimeoutExpired) as exc:
