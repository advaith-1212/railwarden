...
def eligible_providers(
    ...
    *,
    ...
    available = [
        ...
        if name not in excluded and is_available(config.runtime_directory, name)
    ]
    return sorted(
        available,
        key=lambda name: (
...
def hydrate_task_state(files: ProjectFiles) -> list[dict[str, Any]]:
    ...
    for state in states:
        ...
        if state.state == "merged":
            ...
            transition_task(config.runtime_directory, existing_task, "integration_ready")
...
def _handoff_or_block(
    ...
    *,
    ...
    if next_provider:
        return transition_task(config.runtime_directory, task, "handoff_needed", payload)
...
def reconcile_processes(files: ProjectFiles) -> list[dict[str, Any]]:
    ...
    for task in tasks:
        ...
        if package is None:
            transition_task(config.runtime_directory, task, "failed", {"reason": "package removed"})
            continue
        ...
        result_path = Path(str(task.get("result_path", _result_path(config, str(task["id"])))))
        if result_path.exists():
            ...
            if result.get("status") == "completed":
                ...
                transition_task(config.runtime_directory, task, "failed", {"result": result})
            continue
        log_path = Path(str(process.get("log_path", "")))
        failure_text = log_path.read_text(encoding="utf-8", errors="replace") if log_path.exists() else ""
