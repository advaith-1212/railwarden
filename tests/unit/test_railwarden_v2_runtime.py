...
from railwarden.mcp.server import tool_schemas
...
from railwarden.runtime.launch_setups import load_launch_setups
from railwarden.runtime.model_refs import parse_model_ref
from railwarden.runtime.quota import update_usage
from railwarden.runtime.secrets import contains_secret, redacted
...
from railwarden.runtime.skills import create_runtime_skill
from railwarden.runtime.tasks import ensure_task, load_tasks, save_tasks
from railwarden.runtime.typed_agents import create_pydanticai_agent
from railwarden.tmux.session import launch_layout
...
def test_pydanticai_agent_translates_railwarden_model_refs(
...
def test_reset_agent_for_launch_clears_stale_state() -> None:
    ...
    updated = reset_agent_for_launch(agent, model_ref="azure-foundry:deployment-a")
...
def test_hermes_profile_generation_is_runtime_only(git_repo: Path) -> None:
    ...
    assert "RailWarden is authoritative" in hermes.instructions_path.read_text(
        encoding="utf-8"
    )
    assert "Hermes Soul" in hermes.soul_path.read_text(encoding="utf-8")
    assert "name: railwarden-factory" in hermes.skill_path.read_text(encoding="utf-8")
    config = hermes.config_path.read_text(encoding="utf-8")
    assert "mcp_servers:" in config
    assert "command: warden" in config
    ...
    assert mcp["mcpServers"]["railwarden"]["command"] == "warden"
    assert mcp["mcpServers"]["railwarden"]["args"] == ["mcp", "serve"]
...
def test_hermes_profile_inherits_existing_auth(
    ...
    profile = load_session_profile(files.project)
    runtime_auth = (
        files.project.runtime_directory / "hermes" / profile.name / "auth.json"
    )
...
def test_mcp_stdio_server_lists_and_calls_railwarden_tools(git_repo: Path) -> None:
    initialize_project(git_repo, yes=True)
    if os.name == "nt":
        from railwarden.mcp.server import tools

        registered = tools(git_repo)
        assert "railwarden.task.list" in registered
        assert registered["railwarden.task.list"].handler({}) == {"tasks": []}
        return
    ...
    async def probe() -> None:
        ...
        params = StdioServerParameters(
            ...
            args=["-m", "railwarden.cli.main", "mcp", "serve"],
            cwd=git_repo,
        )
        async with (
            ...
            assert "railwarden.task.list" in {tool.name for tool in result.tools}
            response = await session.call_tool("railwarden.task.list", {})
...
def test_tmux_launch_layout_has_factory_and_observability(git_repo: Path) -> None:
    ...
    assert any("warden observability" in spec.command for spec in specs)
    ...
    assert not any("warden worker codex" in spec.command for spec in specs)
    assert any(
        spec.window == "observability" and "warden controller" in spec.command
        for spec in specs
    )
    assert not any("watch -n 5 warden observability" in spec.command for spec in specs)
...
def test_guided_launch_creates_named_setup_and_runtime_env(
    ...
    assert profile.orchestrator.model_profile.model_ref.startswith(
...
def test_low_quota_prevents_new_task_launch(git_repo: Path) -> None:
    ...
    subprocess.run(
        ["git", "-C", str(git_repo), "checkout", "integration/railwarden"],
        check=True,
        capture_output=True,
    )
    files = load_project_files(git_repo)
...
def test_checkpoint_filters_disallowed_files(git_repo: Path) -> None:
    ...
    files = load_project_files(git_repo)
    subprocess.run(
        ["git", "-C", str(git_repo), "checkout", "-b", "railwarden/wp-1"],
        check=True,
        capture_output=True,
    )
    (git_repo / "src").mkdir()
...
def test_worker_prompt_includes_skills_and_mcp_routing(git_repo: Path) -> None:
    ...
    assert "RailWarden MCP routing" in prompt
...
def test_crash_resume_checkpoints_dirty_worktree_and_reassigns(git_repo: Path) -> None:
    ...
    assert message == "warden checkpoint: task-WP-1 attempt 1"
